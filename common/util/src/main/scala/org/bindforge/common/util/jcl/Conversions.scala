...
package org.bindforge.common.util.jcl
...
object Conversions {
   ...
   */
  def iteratorToJavaEnumeration[T](iterator: Iterator[T]): Enumeration[T] =
  iterator match {
    case null => null
   ...
   */
  def iteratorToJavaIterator[T](iterator: Iterator[T]): java.util.Iterator[T] =
  iterator match {
    case null => null
   ...
   */
  def mapToJavaDictionary[K, V](map: Map[K, V]): Dictionary[K, V] =
  if (map == null) null
  else new Dictionary[K, V] {
    override def size = map.size
    override def isEmpty = map.isEmpty
    override def keys = iteratorToJavaEnumeration(map.keys)
    override def elements = iteratorToJavaEnumeration(map.values)
  ...
  /**
   * Implicitly converts from Java enumeration to Scala iterator
   */
  def javaEnumerationToIterator[T](enum: Enumeration[T]): Iterator[T] = {
    enum match {
      case null => null
      case _ => new Iterator[T] {
